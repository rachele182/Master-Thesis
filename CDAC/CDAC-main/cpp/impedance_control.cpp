...
#include <CDAC/impedance_control.h>
...
//                          		INIT										//
//------------------------------------------------------------------------------//
bool ImpedanceControl::init(  hardware_interface::RobotHW* robot_hw, 
	...
	sub_des_traj_ =  node_handle.subscribe(  "/impedance_control/desired_trajectory", 1, 
													&ImpedanceControl::desiredProjectTrajectoryCallback, this,
													ros::TransportHints().reliable().tcpNoDelay());

	sub_des_imp_ =   node_handle.subscribe(  "/impedance_control/desired_impedance", 1, 
													&ImpedanceControl::desiredImpedanceProjectCallback, this,
													ros::TransportHints().reliable().tcpNoDelay());

	sub_ext_forces =      node_handle.subscribe(  "/franka_state_controller/F_ext", 1, 
													&ImpedanceControl::f_ext_Callback, this,
	 										    ros::TransportHints().reliable().tcpNoDelay());


	pub_ext_forces =        node_handle.advertise<geometry_msgs::WrenchStamped>("/impedance_control/ext_forces", 1);
	pub_endeffector_pose_ = node_handle.advertise<geometry_msgs::PoseStamped>("/impedance_control/franka_ee_pose", 1);
	pub_robot_state_ =      node_handle.advertise<panda_controllers::RobotState>("/impedance_control/robot_state", 1);
	pub_impedance_ =        node_handle.advertise<std_msgs::Float64>("/impedance_control/current_impedance", 1);
	pub_info_debug =        node_handle.advertise<panda_controllers::InfoDebug>("/impedance_control/info_debug", 1);
	...
	if (!node_handle.getParam("arm_id", arm_id)) {
		ROS_ERROR_STREAM("ImpedanceControl: Could not read parameter arm_id");
		return false;
	}
	std::vector<std::string> joint_names;
	if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
		ROS_ERROR(
				"ImpedanceControl: Invalid or no joint_names parameters provided, "
				"aborting controller init!");
		return false;
	}
	if (!node_handle.getParam("var_damp", var_damp)) {
		ROS_ERROR_STREAM("ImpedanceControl: Could not read parameter var_damp");
		return false;
	}

	franka_hw::FrankaModelInterface* model_interface = robot_hw->get<franka_hw::FrankaModelInterface>();
	if (model_interface == nullptr) {
		ROS_ERROR_STREAM("ImpedanceControl: Error getting model interface from hardware");
		return false;
	}
	try {
		model_handle_.reset(
		...
		ROS_ERROR_STREAM(
				"ImpedanceControl: Exception getting model handle from interface: "
				<< ex.what());
		return false;
	}
	...
	if (state_interface == nullptr) {
		ROS_ERROR_STREAM("ImpedanceControl: Error getting state interface from hardware");
		return false;
	}
	try {
		state_handle_.reset(
		...
		ROS_ERROR_STREAM(
				"ImpedanceControl: Exception getting state handle from interface: "
				<< ex.what());
		return false;
	}
	...
	if (effort_joint_interface == nullptr) {
		ROS_ERROR_STREAM("ImpedanceControl: Error getting effort joint interface from hardware");
		return false;
	}
	for (size_t i = 0; i < 7; ++i) {
		try {
			...
			ROS_ERROR_STREAM(
					"ImpedanceControl: Exception getting joint handles: " << ex.what());
		return false;
		}
	}
	...
	orientation_d_.setZero();               // desired orientation
...
void ImpedanceControl::starting(const ros::Time& /*time*/) {
...
void ImpedanceControl::update(  const ros::Time& /*time*/,
	...
	// Bx*ddzdes - Bx*inv(Bm)*(Dm*e_dot + Km*e) + (Bx*inv(Bm) - I)*F_ext - Bx*Ja_dot*q_dot;
	...
	// tau = (Ja')*F_tau + S*q_dot + G + tau_fric;
...
//                    	TORQUE SATURATION                    	 //
//---------------------------------------------------------------//
Eigen::Matrix<double, 7, 1> ImpedanceControl::saturateTorqueRate(
...
//----------- DESIRED IMPEDANCE -------------//
void ImpedanceControl::desiredImpedanceProjectCallback(
...
//----------- DESIRED TRAJECTORY -------------//
void ImpedanceControl::desiredProjectTrajectoryCallback(
...
//----------- EXTERNAL FORCES -------------//
void ImpedanceControl::f_ext_Callback(const geometry_msgs::WrenchStampedConstPtr& msg){
...
PLUGINLIB_EXPORT_CLASS(panda_controllers::ImpedanceControl,
