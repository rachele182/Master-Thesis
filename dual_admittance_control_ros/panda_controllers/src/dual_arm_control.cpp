...
//  **************************************************************************
// Purpose: Dynamic Motion Controller Bimanual system
// Description: This represent the inner motion controller for a bimanual system, that tracks the compliant trajectory computed from the outer impedance loop in the presences
//              of external force to guarantee safe interaction.
// Input: desired pose in terms of absolute and relative variables 
// Output: torque vector [N/m]
...
#define 	COLL_LIMIT		300.0    // joint limits collition parameter  
...
// Define Arm parameters for dynamirc identifications:
// _l = left arm , _r = right arm.
...
bool DualArmControl::init( hardware_interface::RobotHW* robot_hw, 
	...
	//--------------- INITIALIZE SUBSCRIBERS AND PUBLISHERS NODES -----------------//
	...
	initial_tau_ext_l.setZero();                    // initial external torque acting on left arm
	initial_tau_ext_r.setZero();                    // initial external torque acting on right arm
...
void DualArmControl::starting(const ros::Time& /*time*/) {
	...
	Map<Matrix<double, 7, 1> > q_l_in(robot_state_left.q.data());    // initial joint positions left arm  [rad]
	Map<Matrix<double, 7, 1> > q_r_in(robot_state_right.q.data());   // initial joint positions right arm [rad]
	Map<Matrix<double, 7, 1> > dq_l_in(robot_state_left.dq.data());  // initial joint velocities left arm  [rad/s]
	Map<Matrix<double, 7, 1> > dq_r_in(robot_state_right.dq.data()); // initial joint velocities right arm  [rad/s]
	//Augmented joint vector
	q_in.head(7) << q_l_in; q_in.tail(7) << q_r_in;

	// ==========  BASE FRAMES (set base of right arm as world frame) ======= ///
	...
	//Load Panda robot DQ model (left arm)
	DQ_SerialManipulator panda_left = init_dq_robot(p_b_0_l,r_b_0_l,0*0.05);

	//Load Panda robot DQ model (right arm)
	DQ_SerialManipulator panda_right = init_dq_robot(p_b_0_r,r_b_0_r,0*0.05);

	//Cooperative dual arm system: bimanual robot 
	DQ_CooperativeDualTaskSpace dual_panda = init_dual_panda(&panda_left, &panda_right);

	//Initial pose EE left arm
	x1_dq = dual_panda.pose1(q_in);
	pos_l_ = vec3(x1_dq.translation()); //EE position
	rot_l_ = vec4(x1_dq.rotation());    //EE rotation

	//Initial pose EE left arm
	x2_dq = dual_panda.pose2(q_in);
	pos_r_ = vec3(x2_dq.translation()); //EE position
	rot_r_ = vec4(x2_dq.rotation());    //EE rotation
	...
	//set equilibrium points to current state for relative and absolute pose
	...
	//initialize momentum observer variables
	r_r.setZero();
        pr_dot_hat.setZero(); 
	pr_int_hat.setZero(); 
	r_l.setZero();
...
// ============= Load Cooperative Dual Task Space Model ======================= //
...
// ------DQ Variables----- //
	...
	J_aug.setZero();                       // inizialize matrix
...
// // ============================== GET ARMS STATE ======================== // //
	...
	Map<Matrix<double, 7, 1> > dq_l(robot_state_left.dq.data());                     // joint velocities  [rad/s]
	//Right arm
	Map<Matrix<double, 7, 1> > q_r(robot_state_right.q.data());                      // joint positions  [rad]
	Map<Matrix<double, 7, 1> > dq_r(robot_state_right.dq.data());  					 // joint velocities [rad/s]
	//Augmented joint vectors q= [q_l;q_r]; dq = [dq_l;dq_r]
...
// //================================ GET DYNAMICS MODEL ================  //
   // Augmented Matrices as e.g Ma = [Ml; Mr]
	...
	Matrix <double, 6,7> Jr_inv;				// dynamically consistenst pseudo-inverse of Jr^T
	Matrix <double, 6,7> Jl_inv;				// dynamically consistenst pseudo-inverse of Jl^T
	Ma.setZero(); Ca.setZero(); 
    // get values from Franka 
	...
	// Eigen conversion from array to MAtrise 
    Map<Matrix<double, 7, 1> > g1(gravity_array_left.data());                  
	...
	// Compute transposes Matrices 
	Jg_l_t = Jg_l.transpose(); Jg_r_t = Jg_r.transpose();

	//----------IDENTIFACTION MATRICEs: DYNAMIC MODEL------------ //
    //*** Description: Panda Dynamics
    //*** Released Date: Apr. 2022
    //*** Author: Mario Tröbinger
    //*** Email: mario.troebinger@tum.de
...
// // ================= GET JACOBIANS ===================== //
	...
	// Compute numerical derivate of relative and absolute Jacobians
	Jr_dot = (Jr - Jr_old)/(period.toSec()); 
	Ja_dot = (Ja - Ja_old)/(period.toSec()); 
	
	// derivative of relative and absolute pose
	dpose_rel << Jr*dq; 
	dpose_abs << Ja*dq; 
    
	// augmented Jacobian and its derivative
...
//---------------- CONTROL COMPUTATION -----------------//
   ...
   tau_task << Ca*dq + Ma*aq + ga_mario - ga;  //remove pre-calculated gravity compensation from frnka ros. 
...
// //----------- TORQUE SATURATION and COMMAND-------------//
	...
	gain_l = KO; //observer Gain Matrix (left arm)
	gain_r = KO; //observer Gain Matrix (left arm)

	Vector7d beta_r,beta_l,pr_int_hat, pl_int_hat;  
	Matrix<double,7,7> Cr_t, Cl_t; 
	Cr_t = c2_mario.transpose(); Cl_t = c1_mario.transpose(); 

	// Momentum of left and right arm 
	p_l = m1_mario*dq_l;  
	p_r = m2_mario*dq_r;
	
	// Remove initial torque bias
	...
	// Compute External Wrench Estimation
...
// Internal Motion Controller subscribe to the compliant trajetory computed from the impedance loop //
